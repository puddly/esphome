...
void USBUartComponent::apply_channel_settings(USBUartChannelBase *channel) {
  this->request_single_(channel, ConfigMode::CONFIG_MODE_RELOAD);
}

void USBUartComponent::apply_modem_control(USBUartChannelBase *channel) {
  this->request_single_(channel, ConfigMode::CONFIG_MODE_MODEM);
}

void USBUartComponent::request_single_(USBUartChannelBase *channel, ConfigMode mode) {
  if (this->cfg_active_) {
    // A config sequence is already running. Defer this run until it finishes to preserve
    ...
    // requests; the channel's live state is read when the run eventually happens.
    // Note: multiple channels are not queued; only one pending request per mode is supported.
    if (mode == ConfigMode::CONFIG_MODE_MODEM) {
...
bool USBUartComponent::run_config_machine_() {
  ...
  if (channel != nullptr && init && !this->cfg_string_done_) {
